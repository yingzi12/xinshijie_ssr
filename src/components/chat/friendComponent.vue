...
getChatList(props.recipientUserId);
...
stompClient.connect().then(() => {
  ...
  const send = props.sendUserId;
  const destination = '/queue/chat/' + receiver+"_"+send;
  stompClient.subscribe(destination, message => {
    console.log('Received message:', message);
    chatList.value.push({message:message.value,sendUserId:props.sendUserId,recipientUserId:props.recipientUserId,sendTime:new Date()});
  });
}).catch((error) => {
